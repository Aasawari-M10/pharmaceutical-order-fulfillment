...
export class CbsClient {
    ...
    async negotiateClaim(audience, token, tokenType, options = {}) {
        ...
        try {
            ...
            logger.warning("[%s] An error occurred while negotiating the cbs claim: %s", this.connection.id, isError(err) ? `${err.name}: ${err.message}` : String(err));
    ...
    async close() {
        try {
            ...
            const msg = `An error occurred while closing the cbs link: ${isError(err) && err.stack ? err.stack : JSON.stringify(err)}.`;
    ...
    remove() {
        try {
            ...
            const msg = `An error occurred while removing the cbs link: ${isError(err) && err.stack ? err.stack : JSON.stringify(err)}.`;
            logger.verbose("[%s] %s", this.connection.id, msg);
            throw new Error(msg, { cause: err });
        }
    }
